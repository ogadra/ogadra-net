locals {
  ...
  selected_google_cloud_domain_name_server_names = slice(sort([
  ...
  google_cloud_domain_name_servers = [
    for name_server in local.selected_google_cloud_domain_name_server_names : {
      name     = name_server
      glue_ips = []
    }
  ]

  domain_name_servers = concat(local.aws_domain_name_servers, local.google_cloud_domain_name_servers)
}
