...
resource "aws_route53_record" "acm_validation_apne3" {
  for_each = {
    for dvo in aws_acm_certificate.apne3.domain_validation_options :
    dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "apne3" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.acm_validation_apne3 : record.fqdn]
}
